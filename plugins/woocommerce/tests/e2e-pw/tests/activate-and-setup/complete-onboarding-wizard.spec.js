...
test.describe( 'Store owner can complete onboarding wizard', () => {
	...
	test.beforeEach( async () => {
		await test.step( 'Complete "Store Details" step through the API to prevent flakiness when run on external sites.', async () => {
	...
	test( 'can complete the "Store Details" section', async ( { page } ) => {
		await test.step( 'Complete store details section using US store details.', async () => {
	...
	test( 'can complete the industry section', async ( { page } ) => {
		await test.step( 'Complete industry section', async () => {
	...
	test( 'can save industry changes when navigating back to "Store Details"', async ( {
		...
		await test.step( `Navigate back to "Store Details" section`, async () => {
	...
	test( 'can complete the product types section', async ( { page } ) => {
		await test.step( `Complete product types section`, async () => {
	...
	test( 'can complete the business section', async ( { page } ) => {
		await test.step( `Ensure that previous steps are complete to avoid generating an error`, async () => {
			await test.step( `Complete industry section`, async () => {
				await onboarding.completeIndustrySection(
					page,
					storeDetails.us.industries,
...
test.describe( 'A Liberian store can complete the selective bundle install but does not include WCPay.', () => {
	...
	test( 'can choose not to install any extensions', async ( { page } ) => {
		...
		await test.step( `Complete product types`, async () => {
			await onboarding.completeProductTypesSection(
				page,
				storeDetails.liberia.products
			);

			await test.step( `Make sure WC Payments is NOT present`, async () => {
				await expect(
					page.locator(
						'.woocommerce-admin__business-details__selective-extensions-bundle__description a[href*=woocommerce-payments]'
					)
		...
		await test.step( `Expect page to navigate away from the business details section`, async () => {
